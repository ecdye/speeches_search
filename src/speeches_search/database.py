...
def _populate_speaker(conn, speaker: Speaker):
    row = conn.execute(
        """
        INSERT INTO speakers (name, bio) VALUES (%s, %s)
    ...
    for talk in speaker["talks"]:
        row = conn.execute(
            """
            INSERT INTO talks (speaker_id, title, date, url)
            VALUES (%s, %s, %s, %s)
        ...
        for i, paragraph in enumerate(paragraphs):
            conn.execute(
                """
                INSERT INTO paragraphs (talk_id, paragraph_index, content)
                VALUES (%s, %s, %s)
...
def get_all_speakers() -> list[dict]:
    with get_connection() as conn:
        rows = conn.execute("SELECT id, name FROM speakers ORDER BY name").fetchall()
        return rows
...
def get_speakers_for_indexing() -> list[Speaker]:
    with get_connection() as conn:
        speaker_rows = conn.execute("SELECT id, name, bio FROM speakers ORDER BY name").fetchall()
        speakers: list[Speaker] = []
        for sr in speaker_rows:
            talk_rows = conn.execute(
                "SELECT id, title, date, url FROM talks WHERE speaker_id = %s ORDER BY title",
                (sr["id"],),
            ).fetchall()
            talks = []
            for tr in talk_rows:
                para_rows = conn.execute(
                    "SELECT content FROM paragraphs WHERE talk_id = %s ORDER BY paragraph_index",
                    (tr["id"],),
                ).fetchall()
                talks.append({
                    "title": tr["title"],
...
def get_existing_talk_titles(speaker_name: str) -> set[str]:
    ...
    with get_connection() as conn:
        rows = conn.execute(
            ...
            FROM talks t
            JOIN speakers s ON t.speaker_id = s.id
            JOIN paragraphs p ON p.talk_id = t.id
            WHERE s.name = %s
            GROUP BY t.title
            """,
            (speaker_name,),
